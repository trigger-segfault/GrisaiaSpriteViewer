...
namespace Grisaia.SpriteViewer {
	class ImageClipboard {
		...
		public static void SetClipboardImage(Bitmap image, Bitmap imageNoTr, string tempFile, DataObject data) {
			...
			using (var ms = (MemoryStream) content.GetData(DataFormats.Dib)) {
				dib = ms.ToArray().Take(80).ToArray();
			}
			using (var ms = (MemoryStream) content.GetData("Format17")) {
				dibv5 = ms.ToArray().Take(150).ToArray();
			}
			Clipboard.Clear();
			if (data == null)
				data = new DataObject();
			...
			using (MemoryStream dibMemStream = new MemoryStream()) {
				...
				//data.SetData(DataFormats.FileDrop, false, 
