	...
	public static partial class Hgx2png {
			...
			string fileName, bool expand)
		{
			...
			if (hdr.Signature != "HG-3")
				throw new UnexpectedFileTypeException(hg3File, "HG-3");
			...
			string fileName, bool expand)
		{
			...
			for (int i = 0; true; i++) {
				...
				while (tag.OffsetNext != 0) {
					...
					string pngFile = Path.Combine(outputDir, MakeFileName(
						Path.GetFileNameWithoutExtension(fileName), i > 0 || hdr.EntryCount > 0, i, imgIndex++));
					...
					/*if (tag.Signature.StartsWith("img_al")) {
						...
						ProcessImage(reader,
							...
							imghdr,
							expand);
						return new Hg3Image(Path.GetFileNameWithoutExtension(pngFile), stdInfo);
					}
					else {
						// Skip this tag
						stream.Position += tag.Length;
