...
namespace Grisaia.Ini {
	...
	internal class MissingTypeException : Exception {
	...
	internal class NoInheritanceException : Exception {
	...
	internal static class TypeHelper {
		...
		/// base interface. Returns null if no interface was found.</summary>
		public static Type GetExtendedInterface(Type type, Type baseInterface) {
			Type nextInterface = baseInterface;
			bool found = false;
			foreach (Type interfaceType in type.GetInterfaces()) {
				if (nextInterface.IsAssignableFrom(interfaceType)) {
					nextInterface = interfaceType;
					found = true;
				}
			}
			return (found ? nextInterface : null);
		}

		/// <summary>Gets the type's most-extended interface that inherits from the
		/// base interface. Returns null if no interface was found.</summary>
		public static Type GetExtendedInterface<BaseInterface>(Type type) {
			return GetExtendedInterface(type, typeof(BaseInterface));
		}
