	...
	public class ImageClipboard2 {
		...
		public static void SetClipboardImage(Bitmap image, Bitmap imageNoTr, string tempFile, DataObject data = null) {
			...
			using (MemoryStream dibv5MemStream = new MemoryStream()) {
				...
				/*image.Save(png2MemStream, ImageFormat.Png);
				data.SetData(DataFormats.FileDrop, false, png2MemStream);*/

				if (tempFile != null)
					data.SetData(DataFormats.FileDrop, true, new string[] { tempFile });
		...
		private static byte[] ConvertToDib(Image image) {
			...
			ArrayUtils.WriteIntToByteArray(fullImage, hdrSize + 0, 4, true, 0x3228001E);
			ArrayUtils.WriteIntToByteArray(fullImage, hdrSize + 4, 4, true, 0xED26001E);
			ArrayUtils.WriteIntToByteArray(fullImage, hdrSize + 8, 4, true, 0xFF35092A);
			return fullImage;
		}
